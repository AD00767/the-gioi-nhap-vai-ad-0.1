  ...
  useEffect(() => {
    const fetchStatsAndData = async () => {
      try {
        ...
        const unmigratedUsers = usersSnap.docs.filter(d => !d.data().numericId || String(d.data().numericId).length !== 9).length;
        const unmigratedChars = charsSnap.docs.filter(d => !d.data().numericId || String(d.data().numericId).length !== 9).length;
        const unmigratedPrompts = promptsSnap.docs.filter(d => !d.data().numericId || String(d.data().numericId).length !== 9).length;
        ...
        toast.error("Không thể tải thông tin thống kê quản trị.");
      } finally {
        setLoading(false);
      }
    };
  ...
  const runIdMigration = async () => {
    ...
    try {
      ...
      setMigrationStats({
        ...
        unmigratedChars: charsSnapUpdated.docs.filter(d => !d.data().numericId || String(d.data().numericId).length !== 9).length,
        unmigratedPrompts: promptsSnapUpdated.docs.filter(d => !d.data().numericId || String(d.data().numericId).length !== 9).length
      });
  ...
  return (
    <AdminLayout>
      <div className="space-y-10 animate-in fade-in duration-700">
        ...
        {quotaError && (
          <div className="p-5 bg-amber-500/10 border border-amber-500/30 text-amber-900 dark:text-amber-200 rounded-3xl flex items-start gap-4 shadow-sm animate-in fade-in duration-300">
            <AlertCircle className="w-6 h-6 text-amber-500 shrink-0 mt-0.5" />
            <div className="space-y-1.5 flex-1">
              <h4 className="font-bold text-sm md:text-base text-amber-600 dark:text-amber-400">Hạn ngạch cơ sở dữ liệu đã hết hôm nay</h4>
              <p className="text-xs md:text-sm text-neutral-600 dark:text-neutral-400 leading-relaxed">
                Hạn ngạch đọc Firestore miễn phí cho dự án này tạm thời bị vượt quá hôm nay. Để bảo đảm trải nghiệm không bị gián đoạn, hệ thống tự động tải dữ liệu thống kê từ mẫu dự phòng dưới đây. Quota sẽ tự động reset vào ngày mai.
              </p>
              <div className="pt-2">
                <a 
                  href="https://console.firebase.google.com/project/gen-lang-client-0874806954/firestore/databases/ai-studio-thgiinhpvaiad01-ccf37355-a8d0-4195-b866-6a33e40f6f39/data?openUpgradeDialog=true"
                  target="_blank"
                  rel="noreferrer"
                  className="inline-flex items-center gap-1.5 px-4 py-2 bg-amber-500 hover:bg-amber-600 text-white dark:text-black font-bold text-xs rounded-xl shadow transition-colors"
                >
                  <span>Nâng cấp/Quản lý Cơ sở dữ liệu</span>
